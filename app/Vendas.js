...
export default function Vendas() {
  ...
  const generatePDF = async () => {
    ...
    const options = {
      html: htmlContent,
      fileName: 'TransacoesMensais',
      directory: 'Documents',
    };

    try {
      const file = await RNHTMLtoPDF.convert(options);
      Alert.alert('PDF Gerado', `PDF salvo em: ${file.filePath}`);
      deleteAllTransactions(); // Exclui as transações após gerar o PDF
    } catch (e) {
      Alert.alert('Erro ao gerar o PDF.');
    }
  };
  ...
  const deleteAllTransactions = async () => {
    try {
      ...
      setTransactions([]);
      Alert.alert('Transações excluídas com sucesso.');
    } catch (e) {
      Alert.alert('Erro ao excluir as transações.');
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      />
      <Button title="Gerar PDF Mensal" color="#E0939D" onPress={generatePDF} />
      </View>
